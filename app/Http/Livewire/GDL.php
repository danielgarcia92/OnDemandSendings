        ...
        if (Auth::user()->rol == 'admin' || Auth::user()->rol == 'jt' || Auth::user()->rol == 'ccv' || Auth::user()->rol == 'gdl') {
            try{
                ...
                $this->flightsHGR = \DB::table('AIMSPROD.VIV.dbo.LEGMAIN AS FL')
                    ...
                    ->where('FL.CANCELLED', '=', 0)
                    ->whereBetween('FL.DAY', [$OQAD->DATE, $OQAD->DATE + 1])
                    ->whereNull(\DB::raw("(SELECT TOP 1 FL2.FLT FROM AIMSPROD.VIV.dbo.LEGMAIN FL2 WITH (NOLOCK) WHERE FL2.DAY BETWEEN ".$OQAD->DATE." AND ".$OQAD->DATE." + 1 AND FL2.REG = FL.REG AND FL2.DEP = 'GDL' AND ((FL2.DAY = FL.DAY AND FL2.STD > FL.STD) OR (FL2.DAY > FL.DAY)) AND ((FL2.DAY > ".$OQAD->DATE." OR FL2.STD > 300) AND (FL2.DAY < ".$OQAD->DATE." + 1 OR FL2.STD < 400)) AND CANCELLED = 0 ORDER BY FL2.DAY, FL2.STD)"))
